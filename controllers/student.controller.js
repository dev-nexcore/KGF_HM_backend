...
const generateRefreshToken = async student => {
  const refreshToken = jwt.sign(
    { sub: student._id.toString(), email: student.email },
    process.env.JWT_SECRET,
    { expiresIn: "30d" }
  );
...
// -------------------- FORGOT PASSWORD --------------------
 const forgotPassword = async (req, res) => {
  ...
  try {
    const student = await Student.findOne({ email });
    if (!student) return res.status(400).json({ message: "Email not recognized" });
...
– Hostel Admin`,
    ...
    return res.json({ message: "OTP sent via Email & WhatsApp" });
  } catch (err) {
    console.error("Forgot password error:", err);
    return res.status(500).json({ message: "Server error during OTP generation." });
  }
...
// -------------------- VERIFY OTP --------------------
 const verifyOtp = async (req, res) => {
  ...
  try {
    const otpDoc = await Otp.findOne({ email, code: otp, purpose: "password_reset" });
    if (!otpDoc) return res.status(400).json({ message: "Invalid OTP" });
    if (otpDoc.expires < new Date()) return res.status(400).json({ message: "OTP expired" });
...
const PASSWORD_REGEX = /^(?=.*\d)(?=.*[^A-Za-z0-9]).{8,}$/;
 ...
 const resetPassword = async (req, res) => {
  ...
  if (!newPassword || !PASSWORD_REGEX.test(newPassword)) {
    return res.status(400).json({
      message: "Password must be at least 8 chars, include 1 number & 1 special char",
    });
  }

  try {
    const otpDoc = await Otp.findOne({ email, code: otp, verified: true, purpose: "password_reset" });
    if (!otpDoc) return res.status(400).json({ message: "OTP not verified or invalid" });
...
const fileComplaint = async (req, res) => {
  ...
  try {
    ...
    if (complaintType === "Maintenance issue") {
      maintenanceDetails = `\n\nMaintenance Details:
- Floor Number: ${floorNumber || "Not specified"}
...
- Type: ${displayType}
- Subject: ${subject}
- Description: ${description}${maintenanceDetails}
- Filed Date: ${new Date().toLocaleDateString("en-IN")}
${
  attachments.length > 0
    ? `- Attachments: ${attachments.length} file(s) attached`
