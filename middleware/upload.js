...
const studentStorage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    ...
    cb(
      null,
      `student_${studentId}_${Date.now()}${path.extname(
        file.originalname
      )}`
    );
  },
...
const parentStorage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    ...
    cb(
      null,
      `parent_${studentId}_${Date.now()}${path.extname(
        file.originalname
      )}`
    );
  },
...
const complaintStorage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    ...
    cb(
      null,
      `complaint_${timestamp}_${randomStr}${path.extname(
        file.originalname
      )}`
    );
  },
...
// Student/Parent document filter
const documentFileFilter = (req, file, cb) => {
  const allowedTypes = [
    "image/jpeg",
    "image/jpg",
...
const mediaFilter = (req, file, cb) => {
  ...
  const mimeAllowed =
    /image\/(jpeg|jpg|png|gif)|video\/(mp4|quicktime|x-msvideo|webm)/;
